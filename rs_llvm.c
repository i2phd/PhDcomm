...
int pp [mm+1] = { 1, 0, 1, 1, 1, 0, 0, 0, 1} ; /* specify irreducible polynomial
 coeffts */
...
/* convert syndrome from polynomial form to index form  */
      if (s[i]!=0)  syn_error=1 ;        /* set flag if non-zero syndrome => err
or */
      s[i] = index_of[s[i]] ;
    } ;
...
/* form new elp(x) */
            ...
            for (i=0; i<=l[u]; i++)
              { elp[u+1][i] ^= elp[u][i] ;
                elp[u][i] = index_of[elp[u][i]] ;  /*convert old elp value to in
dex*/
              }
          }
        u_lu[u+1] = u-l[u+1] ;
            ...
            { err[loc[i]] = 1;       /* accounts for z[0] */
              ...
              if (err[loc[i]]!=0)
               { err[loc[i]] = index_of[err[loc[i]]] ;
                 ...
                 recd[loc[i]] ^= err[loc[i]] ;  /*recd[i] must be in polynomial
form */
               }
            }
            return 1;
